...
class ModelsConfig:
    ...
    def get_llm_config(
        ...
        """Получить LLMConfig с улучшенной логикой."""
        ...
        try:
            ...
            logger.warning(f"Неизвестный агент: {agent_name}")
            agent_type = AgentType.CLASSIFIER
        ...
        # Специальная обработка для ollama
            ...
            # Per-agent model: agent_models.X → env OLLAMA_MODEL → ollama.model → default
            ...
            # Температуры: из YAML (temperatures секция) → хардкод
            ...
            # Определяем max_tokens в зависимости от типа агента
            agent_requirements = {
                "classifier": 1000,
                "relevance": 1000,
                ...
                "summarizer": 2000,
                "rewriter": 500,
                "style_normalizer": 4096,  # Базовое значение, будет адаптировано
            ...
            # АДАПТИВНОЕ УВЕЛИЧЕНИЕ ДЛЯ STYLE_NORMALIZER
            if agent_name == "style_normalizer":
                ...
                logger.info(
                    f"[StyleNormalizer] Установлен адаптивный max_tokens={max_tokens} для режима {self.mode.value}")
        ...
        if not model_info:
            ...
            raise ValueError(f"Модель не найдена для {agent_name} на {provider_name}")
