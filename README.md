...
12. [Streamlit Dashboard (Frontend)](#streamlit-dashboard-frontend)
13. [Порты сервисов](#порты-сервисов)
...
## Спарсить и сохранить одну статью 

```bash
docker compose exec api python run_scraper.py --url https://habr.com/ru/news/1004288/
```
...
Веб-интерфейс для управления статьями, настройками LLM и запуска pipeline.
...
| Страница | Описание |
|----------|----------|
| **📊 Дашборд** | Статистика: всего статей, новостей, средний score, графики по источникам и релевантности |
| **📰 Статьи** | Таблица всех статей с фильтрами (статус, источник, тип, score). Клик → редактор |
| **✏️ Редактор** | 5 вкладок для каждой статьи: Оригинал (read-only), AI-контент (editable), Telegram, SEO, Метаданные |
| **⚙️ Настройки** | Выбор провайдера и модели, температуры агентов, параметры pipeline, запуск/статус, сырой JSON конфиг |

### Настройки (⚙️)

Страница настроек позволяет управлять всей конфигурацией через UI:

- **Провайдер и модель** — выбор из Ollama/Groq/Google/OpenRouter, выбор модели из списка установленных в Ollama, pull новых моделей
- **Температуры агентов** — слайдеры для каждого из 8 агентов + пресеты (Точный / Стандарт / Творческий)
- **Pipeline** — лимит статей, мин. релевантность, мин. score публикации, Telegraph/Telegram toggles, кнопка запуска
- **Сырой конфиг** — прямое редактирование JSON

Настройки сохраняются в `config/settings.json` и автоматически синхронизируются с `config/models.yaml`.

### Редактор статей (✏️)

Каждая статья открывается в 5-табовом редакторе:

| Вкладка | Поля | Editable |
|---------|------|----------|
| 📄 Оригинал | Заголовок, URL, автор, контент, теги, хабы | ❌ Read-only |
| 🤖 AI | Статус, is_news, score, editorial title/teaser/rewritten | ✅ |
| 📱 Telegram | Post text (HTML), cover image, Telegraph URL/HTML | ✅ |
| 🔍 SEO | Title, description, slug, keywords, focus keyword | ✅ |
| ⚙️ Метаданные | Embedding status, vector ID, images, JSON metadata | ✅ |

### Технические детали

- **Порт:** 8501 (настраивается в `docker-compose.yml`)
- **Фреймворк:** Streamlit
- **БД:** Прямое подключение к PostgreSQL (read/write)
- **API:** Интеграция с FastAPI (`http://api:8000`) для запуска pipeline
- **Ollama:** Прямые запросы к `http://ollama:11434` для списка моделей и pull
...
| Сервис | Порт | URL |
|--------|------|-----|
| **Streamlit** (Dashboard) | 8501 | [http://localhost:8501](http://localhost:8501) |
| **FastAPI** (REST API) | 8000 | [http://localhost:8000/docs](http://localhost:8000/docs) |
| **Directus** (CMS) | 8055 | [http://localhost:8055](http://localhost:8055) |
| **Ollama** (LLM) | 11434 | `http://localhost:11434` |
| **Qdrant** (Vector DB) | 6333 | `http://localhost:6333/dashboard` |
| **PostgreSQL** | 5432 | `postgresql://localhost:5432` |
| **Redis** | 6379 | `redis://localhost:6379` |
| **n8n** (Automation) | 5678 | [http://localhost:5678](http://localhost:5678) |
...
news-aggregator-pro/
├── config/
│   ├── models.yaml                    # Конфигурация LLM, agent_models, температуры
│   └── settings.json                  # Настройки из Streamlit UI (авто-генерируется)
├── src/
│   ├── api/routes/
│   │   └── pipeline.py                # HTTP API для pipeline
│   ├── application/ai_services/
...
│   └── scrapers/habr/
│       └── scraper_service.py         # Парсер Habr
├── streamlit_app/                     # Frontend (Dashboard + Editor + Settings)
│   ├── app.py                         # Дашборд, таблица статей, редактор
│   ├── settings_page.py               # Настройки LLM, температуры, pipeline
│   ├── models.py                      # SQLAlchemy модели (копия для контейнера)
│   ├── Dockerfile                     # Streamlit контейнер (порт 8501)
